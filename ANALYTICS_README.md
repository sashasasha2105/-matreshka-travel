...
**Токен:** `7471119413:AAH4xq-T5nBuD__Sdhyzzo8BEZC2VBSwzl0`

## 🚀 Быстрый старт

### 1. Получение Chat ID

Чтобы бот мог отправлять вам уведомления, нужно получить ваш Chat ID:

#### Способ 1: Автоматический
Бот попытается автоматически определить chat_id при первом запуске приложения. Для этого:

1. Откройте бота в Telegram: [@YourAnalyticsBot](https://t.me/your_analytics_bot)
2. Нажмите `/start`
...
3. Откройте файл `analytics.js`
4. Найдите строку:
   ```javascript
   const ANALYTICS_CHAT_ID = '';
   ```
5. Вставьте ваш Chat ID:
   ```javascript
   const ANALYTICS_CHAT_ID = '123456789'; // Ваш Chat ID
   ```
...
### 1. Запуски бота
При каждом запуске приложения отправляется уведомление с:
- 👤 Информация о пользователе (ID, никнейм, имя)
- 📱 Данные устройства (платформа, размер экрана)
- 🌍 Локация (язык, часовой пояс)
- ⏰ Время запуска
- 🆔 ID сессии
...
Приложение уже настроено для работы на Railway. Аналитика будет работать автоматически.

### Переменные окружения (опционально)

Если хотите управлять настройками через переменные окружения Railway:

1. Откройте проект на Railway
2. Перейдите в Variables
3. Добавьте:
   ```
   ANALYTICS_BOT_TOKEN=7471119413:AAH4xq-T5nBuD__Sdhyzzo8BEZC2VBSwzl0
